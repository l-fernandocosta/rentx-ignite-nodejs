...
export async function ensureAuthenticated(
  ...
  /* We split because the request.headers.authorization returns the default JSON pattern ->  'Bearer 1238193819238'
  ...
  */
  ...
  try {
    const { sub: user_id } = verify(token, process.env.TOKEN_SIGNATURE);
